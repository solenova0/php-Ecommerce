...
function doInsert(){
    ...
    if(isset($_POST['submit'])){
        ...
        if(!isset($_POST['proid']) || (isset($_POST['proid']) && empty($_POST['proid']))){
            ...
            $proid = $_GET['proid'];
            $id = $mydb->pdo->lastInsertId(); 
            $query ="INSERT INTO `tblwishlist` (`PROID`, `CUSID`, `WISHDATE`, `WISHSTATS`) VALUES (:proid, :cusid, :date, 0)";
            $mydb->setQuery($query);
            $mydb->executeQuery([':proid' => $proid, ':cusid' => $id, ':date' => date('Y-m-d')]);
...
function doDelete(){
    if(isset($_SESSION['U_ROLE']) && $_SESSION['U_ROLE']=='Customer'){
...
function processwishlist(){
    ...
    if(isset($_GET['wishid'])){
        $query ="UPDATE `tblwishlist` SET `WISHSTATS`=1 WHERE `WISHLISTID`= :wishid";
        $mydb->setQuery($query);
        $res = $mydb->executeQuery([':wishid' => $_GET['wishid']]);
        if ($res){
...
function addwishlist(){
    ...
    $query = "SELECT * FROM `tblwishlist` WHERE CUSID = :cusid AND PROID = :proid";
    $mydb->setQuery($query);
    $res = $mydb->executeQuery([':cusid' => $id, ':proid' => $proid]);
    $maxrow = $res ? $res->rowCount() : 0;
