...
class Customer {
    ...
    function find_customer($id = "", $name = "") {
        global $pdo;
        $stmt = $pdo->prepare("SELECT * FROM " . self::$tblname . " WHERE CUSTOMERID = :id OR FNAME = :name");
    ...
    function find_all_customer($name = "") {
        global $pdo;
        $stmt = $pdo->prepare("SELECT * FROM " . self::$tblname . " WHERE FNAME = :name");
    ...
    static function cusAuthentication($email, $h_pass) {
        global $pdo;
        $stmt = $pdo->prepare("SELECT * FROM " . self::$tblname . " WHERE CUSUNAME = :email AND CUSPASS = :pass");
        $stmt->execute([':email' => $email, ':pass' => $h_pass]);
        if ($stmt->rowCount() == 1) {
    ...
    function single_customer($id = "") {
        global $pdo;
        $stmt = $pdo->prepare("SELECT * FROM " . self::$tblname . " WHERE CUSTOMERID = :id LIMIT 1");
        $stmt->execute([':id' => $id]);
    ...
    function find_phone($phone = "") {
        global $pdo;
        $stmt = $pdo->prepare("SELECT * FROM " . self::$tblname . " WHERE PHONE = :phone LIMIT 1");
    ...
    public function update($id = 0) {
        ...
        $sql = "UPDATE " . self::$tblname . " SET " . implode(", ", $attribute_pairs) . " WHERE CUSTOMERID = :id";
        $attributes['id'] = $id;
        $stmt = $pdo->prepare($sql);
    ...
    public function delete($id = 0) {
        global $pdo;
        $stmt = $pdo->prepare("DELETE FROM " . self::$tblname . " WHERE CUSTOMERID = :id LIMIT 1");
