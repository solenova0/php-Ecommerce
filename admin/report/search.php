...
if(isset($_POST['submit'])){
    ...
    $query = "SELECT * FROM `tblcustomer` c, `tblsummary` s WHERE c.`CUSTOMERID` = s.`CUSTOMERID` AND s.`ORDEREDSTATS`='Confirmed' AND date(s.`ORDEREDDATE`)>=:start AND date(s.`ORDEREDDATE`)<=:end";
    ...
    if(count($rows) > 0){
        $overall = 0; // initialize overall sum
        foreach($rows as $row){
            echo '<tr>';
